...
class CameraLogger:
    ...
    def open(self) -> None:
        ...
        if str(self.camera_id).lower() == "realsense":
            ...
            config = rs.config()
            # Request RGB8 format at 30 FPS
            config.enable_stream(rs.stream.color, self.resolution[0], self.resolution[1], rs.format.rgb8, 30)
            ...
            logger.info("RealSense camera opened. Resolution: %d×%d", self.resolution[0], self.resolution[1])
            self._is_open = True
            return
    ...
    def capture_frame(self) -> np.ndarray:
        ...
        if str(self.camera_id).lower() == "realsense":
            ...
            # Format is already RGB8 as requested during config
...
with open("/home/kaurav/Openvla verification/work/ur5_data_collection/src/camera_logger.py", "w") as f:
    f.write(content)
